      ...
      try {
        if (admin) {
          const response = await protectedApiGetAdmin(
            `admin/referral/getUsersWithReferralCode`
          );
                  ...
                  <tbody>
                    {currentData.map((data, index) => (
                      <tr key={index} className="border-b">
                        ...
                        <td className="py-2 px-4 description text-sm md:text-base text-[#6D6A6A]">
                          {data.influencerCommission}
                        </td>
                        <td className="py-2 px-4 description text-sm md:text-base text-[#6D6A6A]">
                          {data.userCommission}
                        </td>
                        <td className="py-2 px-4 description text-sm md:text-base text-[#6D6A6A]">
  {data?.date ? new Date(data.date).toLocaleString("en-IN", {
    year: "numeric",
    month: "short",
    day: "numeric",
    hour: "numeric",
    minute: "numeric",
            ...
            <div className="flex justify-between mt-4 text-sm text-gray-600 w-full description">
              <div>
                Showing {startIndex + 1}-{Math.min(endIndex, dataArray.length)}{" "}
                of {dataArray.length}
              </div>
