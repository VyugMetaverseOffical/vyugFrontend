...
const AllNews = ({ search }) => {
  ...
  const filterBlogs = (news, search) => {
    if (!search) return news;
    return news.filter((blog) =>
      blog.heading.toLowerCase().includes(search.toLowerCase())
    );
  };

  const filteredItem = filterBlogs(news, search);
  ...
  return (
    <div className="w-full mt-10 flex flex-col gap-10">
      {filteredItem.length > 0 ?
        filteredItem.slice(0, visibleCount)?.map((item, index) => (
          <div
            key={index}
            onClick={() =>
            ...
            className="cursor-pointer w-full mx-auto transition duration-500 ease-in-out hover:bg-[#EEEEEF] p-2 rounded-2xl flex gap-5"
          >
            ...
            <div className="w-[80%] md:w-[85%] flex flex-col sm:flex-row sm:items-center justify-stretch gap-2 border-l-[1px] border-black pl-5 md:pl-10">
              ...
              <div className="w-full sm:w-[65%] lg:w-[65%] flex flex-col mt-4 sm:mt-0 gap-2 sm:gap-4">
                ...
                <Typography
                  variant="h2"
                  className="header font-bold text-[#111113] sm:font-extrabold text-base text-left sm:text-lg lg:text-2xl w-full"
                >
        ...
        : 
        <Typography variant="h2" className="header text-[#242424] text-center text-xl md:text-2xl w-full">
          No results found <br />
          <span className="text-[6D6A6A] text-sm sm:text-base font-normal">Try using different spelling or keywords !</span>
        </Typography>
      }
